    ...
    <div class="flex flex-col md:flex-row gap-6 md:gap-8">
        ...
        <aside class="w-full md:w-72 flex-shrink-0">
            <div class="rounded-2xl md:rounded-[2rem] border border-line bg-surface p-4 md:p-6 shadow-sm">
                ...
                <h2 class="text-lg md:text-xl font-display font-bold uppercase tracking-[-0.01em] text-bone mt-1 md:mt-3">Categorías</h2>
                ...
                <ul class="flex md:flex-col gap-2 overflow-x-auto pb-2 md:pb-0 mt-4 md:mt-6 no-scrollbar -mx-2 px-2 md:mx-0 md:px-0">
                    <li class="flex-shrink-0">
                        <a href="{{ route('shop.catalog') }}"
                           class="inline-block md:block px-4 py-2 text-xs md:text-sm font-medium rounded-full transition-colors whitespace-nowrap {{ request('category') ? 'text-bone-dim hover:bg-surface-2 border border-transparent' : 'bg-venom text-ink shadow-sm' }}">
                            Todas
                        </a>
                    </li>
                    ...
                    <li class="flex-shrink-0">
                        <a href="{{ route('shop.catalog', ['category' => $category->id]) }}"
                           class="inline-block md:block px-4 py-2 text-xs md:text-sm font-medium rounded-full transition-colors whitespace-nowrap {{ (string) request('category') === (string) $category->id ? 'bg-venom text-ink shadow-sm' : 'text-bone-dim hover:bg-surface-2 border border-transparent' }}">
                            {{ $category->name }}
                        </a>
                    </li>
                    @endforeach
        ...
        <div class="flex-1">
            ...
            <div class="flex flex-wrap items-center justify-between gap-3 mb-6 md:mb-8">
                <div>
                    <span class="text-[10px] md:text-[11px] font-bold uppercase tracking-[0.24em] text-venom">Catálogo</span>
                    <h1 class="text-2xl md:text-4xl font-display font-bold uppercase tracking-[-0.02em] text-bone mt-1">
                        Colección
                        @if(request('search'))
                            <span class="text-bone-dim font-normal normal-case block sm:inline text-base md:text-2xl">— resultados para "{{ request('search') }}"</span>
                        @endif
                    </h1>
                </div>
