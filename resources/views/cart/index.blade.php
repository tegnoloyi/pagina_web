    ...
    <section class="relative min-h-[420px] md:min-h-[520px] flex items-center justify-center w-full max-w-full overflow-x-hidden px-4 sm:px-6 lg:px-8 py-16 text-center">
        <div class="max-w-3xl mx-auto w-full max-w-full px-4 sm:px-6 lg:px-8">
            <div class="w-full flex flex-col items-center">
                <div class="inline-flex items-center gap-2 rounded-full border border-line px-4 py-2 text-[11px] font-bold uppercase tracking-[0.22em] text-venom bg-surface/60 backdrop-blur">
...
<section class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-16">
    ...
    <div class="grid grid-cols-2 md:grid-cols-4 gap-6">
        ...
        <a href="{{ route('shop.catalog', ['category' => $category->id]) }}" class="group block">
            <div class="aspect-square rounded-[2rem] overflow-hidden bg-surface mb-4 border border-line group-hover:-translate-y-1 group-hover:border-venom/40 transition-all duration-300 flex items-center justify-center">
...
<section class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 pb-20">
    ...
    <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-8">
        ...
        <article class="group relative flex flex-col rounded-[2rem] border border-line bg-surface p-3 shadow-sm hover:shadow-lg hover:shadow-venom/5 hover:-translate-y-1 hover:border-venom/30 transition-all duration-300">
            ...
            <div class="flex-grow px-2 py-5">
                ...
                <div class="flex items-center gap-1.5 mt-4">
                    @foreach($product->variants->unique('color') as $variant)
                        <span class="w-3.5 h-3.5 rounded-full border border-line shadow-sm" style="background-color: {{ $variant->color_hex ?? '#ccc' }};" title="{{ $variant->color }}"></span>
                    @endforeach
                </div>
            </div>
        </article>
